/* @ts-self-types="./deal_core.d.ts" */

/**
 * @param {string} raw_text
 * @returns {any}
 */
export function parseRawEmail(raw_text) {
    const ptr0 = passStringToWasm0(raw_text, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.parseRawEmail(ptr0, len0);
    return ret;
}
 ...
 * @returns {any}
 */
export function validateWebsiteExport(rows) {
    const ret = wasm.validateWebsiteExport(rows);
    return ret;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('deal_core_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
